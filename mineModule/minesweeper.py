...
class Board:
  def __init__(self, dim_size, num_bombs):
    ...
    # creating the board using a helper function
  ...
  def assign_values_to_board(self):
    ...
    for r in range(self.dim_size):
      for c in range(self.dim_size):
        if self.board[r][c] == '*':
          continue # if this is alredy a bomb, we don´t want to calculate anything
        else:
...
def play(dim_size=10, num_bombs=10):
  ...
  while len(board.dug) < board.dim_size ** 2 - num_bombs:
    print(board)
    #getting a input like '0, 3', iwth regex to clean spaces after ,
  ...
  if safe:
    ...
    print("Well, it's nothing very special. Try and be nice to people, avoid eating fat, read a good book every now and then, get some walking in, and try and live together in peace and harmony with people of all creeds and nations.\n")
    print(board)
  else:
    print("\nI have come for you. \nTo take you away. \nThat is my purpose. \nI am Death.\n")
...
#get the input for board size and number of bombs
def menu():
  dim = int(input("Choose the board size (default 10): "))
  bombs = int(input("Choose the number of bombs (default 10): "))
